...
/// Class đóng gói dữ liệu thông báo để hiển thị UI động
class MedicationNotice {
  final String message;
  final Color color;
  final Widget? banner;
...
class _GiaoDiNChNhState extends State<GiaoDiNChNh> {
  ...
  void initState() {
    super.initState();
    _now = DateTime.now();
    // Cập nhật mỗi giây để đồng bộ đồng hồ và kiểm tra lịch thuốc
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (mounted) setState(() => _now = DateTime.now());
    });
  }
  ...
  /// Lấy ID người con từ Firestore của User hiện tại
  Future<String?> _getChildIdFromFirestore() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return null;
    final doc = await FirebaseFirestore.instance
  ...
  /// Định dạng ngày tháng tiếng Việt (Thứ..., ngày/tháng/năm)
  String _formatFullDate(DateTime dt) {
    const days = [
      'Chủ Nhật',
      'Thứ Hai',
    ...
    return "${days[dt.weekday % 7]}, ${dt.day.toString().padLeft(2, '0')}/${dt.month.toString().padLeft(2, '0')}/${dt.year}";
  }
        ...
        builder: (context, idSnapshot) {
          ...
          return StreamBuilder<List<SchedulePill>>(
            stream: Stream.fromFuture(
              pillVM.getSchedulePillsByParentIdStream(),
            ).asyncExpand((s) => s),
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                _localPills = snapshot.data!;
            ...
            children: [
              ...
              _buildActionButton(
                ...
                onTap: () {
                  if (notice.currentPillId != null) {
                    try {
                      // 1. Tìm pill tương ứng từ danh sách local dựa trên ID của thông báo
                      final currentPill = _localPills.firstWhere(
                        (p) => p.id == notice.currentPillId,
                      );

                      // 2. Truyền THÊM tham số frequency vào đây
                      vm.confirmPillTaken(
                        notice.currentPillId!,
                        frequency: currentPill.frequency,
                      );
                    } catch (e) {
                      print("Không tìm thấy thông tin thuốc: $e");
                    }
                  }
                },
              ),
  ...
  Widget _buildNotificationBanner({
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.notifications_active, color: Colors.white, size: 45),
          const SizedBox(width: 15),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  void _showSOSDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text("Gửi SOS?"),
        content: const Text(
          "Cảnh báo khẩn cấp sẽ được gửi ngay đến điện thoại của con bạn.",
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  Future<void> _sendSOSAlert() async {
    try {
      ...
      await FirebaseFirestore.instance.collection('notifications').add({
        'receiverId': childId,
        'senderId': user.uid,
        'senderName': userDoc.data()?['name'] ?? "Bố/Mẹ",
        'type': 'SOS',
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Đã gửi tín hiệu SOS!"),
  ...
  Future<void> _sendCallNotification() async {
    try {
      ...
      await FirebaseFirestore.instance.collection('notifications').add({
        ...
        'senderName': userDoc.data()?['name'] ?? "Bố/Mẹ",
        'type': 'CALL',
        'message': 'Bố Mẹ muốn bạn gọi lại.',
        'timestamp': FieldValue.serverTimestamp(),
        'isRead': false,
      });
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Đã gửi yêu cầu gọi con"),
